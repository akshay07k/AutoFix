...
const Header: React.FC = () => {
  ...
  const hideContent = location.pathname === "/login" || location.pathname === "/signup";
  ...
  return (
        ...
        <div className="container flex h-16 items-center gap-4">
          <div className="flex items-center gap-6">
            <Link to="/" className="flex items-center space-x-2">
              <span className="text-xl md:text-2xl font-extralight tracking-wider text-white">
                Auto<span className="font-semibold text-blue-500">Fix</span>
              </span>
            </Link>
          </div>

          {!hideContent && <MainNav className="ml-16 hidden lg:flex" />}
    ...
    return parsedUser?.name?.[0]?.toUpperCase() || "U";
  })()}</AvatarFallback>
                </Avatar>
              </button>
            </div>
